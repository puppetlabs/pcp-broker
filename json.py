...
class JsonReader(object):
    ...
    def _readString(self):
        ...
        # Other code expects byte strings.
...
class JsonWriter(object):
    ESCAPE_TABLE = { chr(i) : r'\u%04x' % i for i in xrange(32) }
    ESCAPE_TABLE['\b'] = r'\b'
    ESCAPE_TABLE['\f'] = r'\f'
    ESCAPE_TABLE['\n'] = r'\n'
    ESCAPE_TABLE['\r'] = r'\r'
    ESCAPE_TABLE['\t'] = r'\t'
    ESCAPE_PATTERN = re.compile('|'.join(re.escape(k)
                                         for k in ESCAPE_TABLE.keys()))


    def __init__(self):
        self._results = None
        self._escaped_forward_slash = None
        self._double_quote_safe = None

    def _append(self, s):
        self._results.append(s)

    def write(self, obj, escaped_forward_slash=False, double_quote_safe=False):
        self._escaped_forward_slash = escaped_forward_slash
        self._double_quote_safe = double_quote_safe
        self._results = []
        self._write(obj)
    ...
    def _write(self, obj):
        ...
        if ty is types.DictType:
            ...
            obj = self.ESCAPE_PATTERN.sub(
                lambda m: self.ESCAPE_TABLE[m.group(0)], obj
            )
...
def write(obj, escaped_forward_slash=False, double_quote_safe=False):
    return JsonWriter().write(obj, escaped_forward_slash, double_quote_safe)

def read(s):
    return JsonReader().read(s)

def swrite(obj, escaped_forward_slash=False):
    return '/*-secure-%s*/' % JsonWriter().write(obj, escaped_forward_slash)

def sread(s):
    return JsonReader().read(s[10:-2])

def ify(func):
    def anonymous(self, *args, **kwargs):
        return write(func(self, *args, **kwargs))
    return anonymous
